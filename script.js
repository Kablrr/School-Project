...
generateBtn.addEventListener("click", () => {
  ...
  const fullPrompt = `${basePrompt}, ${avatarOptions}, colonial America, 18th century, oil painting, parchment texture, historical lighting`;
  ...
  img.src = `https://image.pollinations.ai/prompt/${encodeURIComponent(fullPrompt)}?seed=${Date.now()}`;
  img.alt = fullPrompt;
...
submitBtnEl.onclick = () => {
  ...
  [...answersEl.children].forEach((b,i)=>{
    ...
    if(i===selectedAnswer && i!==correct) b.classList.add("wrong");
  });

  if(selectedAnswer===correct) correctSound.play();
